...
@click.option("-d", "--delete", is_flag=True, default=False, help="Delete BloxOne AuthNSG")
...
def create_auth_nsg(b1ddi, host, ansgname, comment):
    ...
    ansgBody = {"comment": comment, "internal_secondaries": niosx_hosts, "name": ansgname}
    response = b1ddi.create("/dns/auth_nsg", body=json.dumps(ansgBody))
    if response.status_code == 200:
        nsg_response = response.json()
        print(nsg_response["result"])
    else:
        print(response.status_code, response.text)
